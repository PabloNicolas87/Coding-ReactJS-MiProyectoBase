...
# 2f) Workflow: quitar builder si existe
sed -i '/name: 🔨 Build & Push BUILDER image/,/push:/d' "$WORKFLOW"

#  Insertar paso runtime justo después del login
sed -i '/uses: docker\/login-action/a\
